...
class USKELMModel:
    ...
    def predict(self, X, clustering=False, k=None):
        """
            ...
            Returns:
            -----------
                array: Predicted embeddings, or tuple (embeddings, cluster_labels) if clustering=True.
            ...
            >>> pred = model.predict(X)
        """
        result = self.layer.predict(X, clustering, n_clusters=k)
        if clustering:
            # Returns tuple (output, cluster_labels)
