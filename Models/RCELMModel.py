...
class RCELMModel(BaseEstimator, RegressorMixin):
    """
        ...
        According to the literature (Chen et al., 2020 - "Residual compensation extreme
        learning machine for regression"), RC-ELM was originally designed for REGRESSION
        tasks where the prediction error is compensated layer by layer.

        For classification, it can be used but the output needs to be processed with argmax.
