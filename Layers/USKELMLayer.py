...
class USKELMLayer:
    """
        Unsupervised Kernel ELM (USK-ELM) Layer.
        ...
        For classification/regression, use KELMLayer with KELMModel instead.

        According to the literature, Unsupervised ELM/KELM was designed for UNSUPERVISED
        ...
        regularization to preserve data structure in non-linear feature space.
    ...
    def predict(self, x, clustering=False, n_clusters=None):
        """
            ...
            - n_clusters: Number of clusters if clustering is True.
        ...
        """
        x = tf.cast(x, dtype=tf.float32)
        k_matrix = calculate_pairwise_distances_vector(x, self.input, self.kernel.ev)
        kpKT = tf.matmul(k_matrix, self.K)
