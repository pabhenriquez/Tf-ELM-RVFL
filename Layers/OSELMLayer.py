...
class OSELMLayer:
    ...
    def fit_initialize(self, x, y):
        ...
        # For online sequential learning, P must always be number_neurons x number_neurons
        # to allow sequential updates with Woodbury identity
        # P = (H'H + C*I)^(-1)
        HTH = tf.matmul(H, H, transpose_a=True)
        P = tf.linalg.set_diag(HTH, tf.linalg.diag_part(HTH) + self.C)
        P = tf.linalg.inv(P)
