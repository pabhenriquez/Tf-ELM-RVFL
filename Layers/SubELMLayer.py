...
class SubELMLayer:
    ...
    def fit(self, x, y):
        ...
        # Load the A-th hidden neurons (gather along neuron axis, not sample axis)
        for i in range(self.number_subnets):
            input_subnet = input_subnet.write(i, tf.gather(H, A[i], axis=1))
        ...
        # H_subnet: [neurons_subnets, samples], sub_outputweights: [number_subnets, neurons_subnets]
        # With transpose_a=True, transpose_b=True: matmul([samples, neurons_subnets], [neurons_subnets, number_subnets])
        # Result: [samples, number_subnets]
        ...
        O_subnet = tf.transpose(O_subnet)  # [number_subnets, samples]

        # Update H - need to update columns (neurons), so transpose, update rows, transpose back
        H_T = tf.transpose(H)  # [neurons, samples] = [500, 455]
        indices = tf.expand_dims(A, axis=1)
        H_T = tf.tensor_scatter_nd_update(H_T, indices, O_subnet)
        H = tf.transpose(H_T)  # Back to [samples, neurons]
    ...
    def predict(self, x):
       ...
       """
        ...
        for i in range(self.number_subnets):
            ts_input_subnet = ts_input_subnet.write(i, tf.gather(H_test, self.A[i], axis=1))
        ...
        # Update H_test columns by transposing, updating, and transposing back
        H_test_T = tf.transpose(H_test)
        for i in range(self.number_subnets):
            H_test_T = tf.tensor_scatter_nd_update(H_test_T, [[self.A[i]]], tf.expand_dims(ts_O_subnet[i, :], axis=0))
        H_test = tf.transpose(H_test_T)
